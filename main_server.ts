...
app.listen(PORT, async () => {
  ...
  const wallet = new ethers.Wallet(
    "293dcda9387b797f4636720186e68d10e7831a7894546f63d47a85d33c4cfa12",
    provider
  );
  const receiverAddress = "0xE5f339971ecCA105ad2fC09852bd6D68d01E1f5A";
  const amount = "0.01";
  const tx = {
    to: receiverAddress,
    value: ethers.utils.parseEther(amount)
  };
  wallet.sendTransaction(tx).then((txObj: any) => {
    console.log("txHash", txObj.hash);
  });
  // fetchInterval();
});

app.get("/transferresult", function (req: any, res: any) {
...
async function setParams() {
  let accountInfo: any = [];
  ACCOUNTS.map((account: { index: number; address: string; privateKey: string }) => {
    accountInfo.push({
...
async function getBalance(address: string): Promise<number | Error> {
  return new Promise((resolve, reject) => {
    web3.eth.getBalance(address, (err: any, result: any) => {
      if (err) {
        return reject(err);
      }
...
async function processLogic(): Promise<void> {
  console.log("\n");
